...
const ShippingAddressForm = ({ onComplete }: ShippingAddressFormProps) => {
  ...
  const onSubmit = (values: FormData) => {
    // include selected location in submission
    if (location) values.location = location;
    onComplete(values);
  };

  const handleLocationChange = (loc: { lat: number; lng: number; name: string }) => {
    setLocation(loc);
    form.setValue("location", loc); // update react-hook-form field
  };
