...
//   const getLabel = (typeHint, isArabic: boolean): string =>
...
//   const getAllTypeHints = ()=> Object.values({});
...
export const useTypeHints = () => {
  ...
  const getIcon = (typeHint: string) => {
    return typeHintIcons[typeHint];
  };

  const getGradientStyle = (typeHint: string) => {
    const colors = typeHintGradients[typeHint];
    if (!colors) return {};
